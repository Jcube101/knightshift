...
- **Shared navigation:** Home, Play, Learn, and Settings are the only top-level destinations, shown as a visible horizontal top row on both mobile and desktop. Do not use a hamburger drawer at this scope. Review and the full saved-game archive are addressable detail routes reached from Home, not destinations competing for persistent attention. Learn is an independent opening-study workspace, not a Home card, because it supports repeated exploration and persistent repertoire context.
- **Home:** Lead with Resume game when an active game exists, otherwise Start a game. Home is the personal study dashboard: label the entry point `YOUR CHESS STUDY` and use `Your next game` as its heading; do not repeat the Play route label inside the card. Show at most one latest lesson, one quiet pattern signal, a short chronological list of recent games, and a restrained route into Learn. The full archive is available through a restrained All saved games link. Do not show a grid of trend cards, scores, or all pattern totals.
- **Learn:** Introduce opening families and named variations through a guided, board-first explorer. Begin each study path by choosing White or Black. In Black study, organise defenses by White’s first move, label the learner’s reply ...[truncated]
- **Learn catalogue layout:** At multi-column widths, each opening-family card must size to its own content. Do not stretch a short family card to match a taller neighbour, which creates dead space and weakens the quiet reading rhythm.
- **Play:** Board first. Material sits above the board as a compact score, beside a horizontally scrolling move strip with no visible scrollbar. The Engine container is absent from Play. Side selection uses a compact White/Black segmented toggle and difficulty uses an accessible three-step slider, both kept visually recessive beside the board. Undo and resignation are secondary actions: Resign game uses a muted danger treatment, requires confirmation, saves a local loss by resignation, and never competes with New game as the primary action. Side and difficulty are durable defaults configured in Settings and copied into a new game; Play may show the chosen values and allow intentional game-specific changes only if the UI makes that exception explicit.
- **Review:** Open on critical moment I. Keep moments in chronological game order, labelled only I, II, III. The board before the move, red/green overlays, SAN recommendation, and one factual consequence form one focused lesson. While a post-game review is incomplete, show a direct saved-progress count and permit resuming later. A final player move that ends the game, through checkmate, stalemate, or draw, is saved without waiting for a nonexistent engine reply. When it completes from Play, keep the person on Play and offer `Open review` rather than redirecting them. Never imply a browser Worker continues after the tab closes.
- **History:** Present saved games as a chronological archive. Opening names and results are descriptive context, not causal performance claims. Show a recognised line’s first recorded critical moment only as occurring during or after that line, never as proof that the opening caused the result. Pattern summaries are concise and linked to evidence. Avoid aggregate charts until they serve a specific reflective question better than a simple count and saved-game link.
- **Settings:** Keep new-game defaults here. The fixed Quiet Study palette is visible as product identity, not a user preference. The page is intentionally quiet and must not expose game controls or engine telemetry. Private sync belongs here only: a signed-out browser sees a compact email-and-password sign-in form with `Sign in to bring your chess study to this device.`; a signed-in browser sees factual sync status, an explicit Sync now action, and Sign out. No account controls appear on Home, Play, Learn, or the board.
...
- **Learn earns the fourth primary destination:** The generic 2–4 route guidance remains a ceiling, not a requirement to fill four slots. Learn now meets the test for a top-level workspace: it is visited independently of a current game, needs persistent learning and repertoire state, and will be revisited for deliberate study. History remains archive context, so it stays addressable from Home without permanent navigation weight. The four-item visible top row remains comfortable at phone width; reconsider a drawer only if the live workspace count grows beyond four.
- **Visible top row over a hamburger drawer:** With four stable destinations, visible navigation remains faster to scan and switch, remains comfortable at phone width, and makes Knightshift feel like a contained private workspace. A drawer would add a tap to every route change and imply hidden complexity that the app does not have. Reconsider a hamburger drawer only if the live top-level workspace count grows beyond four.
- **Guided opening decisions over longer reference lines:** A short static variation answers “what is this called?” but does not answer “what do I do next?” Learn therefore shows an explicit learner side, a short main path, and named common ...[truncated]
