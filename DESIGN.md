...
- **Shared navigation:** Home, Play, History, and Settings are the only top-level destinations. Review is an addressable detail route reached from Home or History, not a fifth destination competing for attention.
- **Home:** Lead with Resume game when an active game exists, otherwise Play a game. Show at most one latest lesson and one quiet progress signal. Do not show a grid of trend cards, scores, or all pattern totals.
- **Play:** Board first. The move list, material, capture note, difficulty, and side controls support the game without becoming a dashboard. Side and difficulty are durable defaults configured in Settings and copied into a new game; Play may show the chosen values and allow intentional game-specific changes only if the UI makes that exception explicit.
...
- **Settings:** Keep themes and new-game defaults here. The page is intentionally quiet and must not expose game controls, engine telemetry, or account-like preferences.
...
- **Four-route system, not four visual identities:** Shared navigation, palette, typography, spacing, panel behaviour, action hierarchy, and review semantics bind Home, Play, History, and Settings. Per-screen conventions exist only where role demands them: board dominance on Play and Review, next-action dominance on Home, chronology on History, and form simplicity on Settings.
...
- **Durable defaults belong in Settings:** Theme, preferred side, and difficulty are stable preferences. Keeping them in Settings lets Play remain focused on the game and prevents configuration from competing with the board.
