...
- **Shared navigation:** Home, Play, and Settings are the only top-level destinations, shown as a visible horizontal top row on both mobile and desktop. Do not use a hamburger drawer at this scope. Review and the full saved-game archive are addressable detail routes reached from Home, not destinations competing for persistent attention. The three-item navigation is intentional: Home absorbs contextual study material, rather than adding a destination for every capability.
...
- **Three-item primary navigation overrides the generic 2–4 route guidance:** The generic guidance is a ceiling for stable workspaces, not a requirement to fill four slots. For Knightshift, Home, Play, and Settings are sufficient. History is archive context rather than an independent workspace at the current scope, so its recent-game and pattern summary belong on Home. The full archive remains at an addressable route through All saved games, preserving depth without permanent navigation weight. Reconsider a fourth destination only when a task becomes an independently visited, persistent workspace.
- **Visible top row over a hamburger drawer:** With exactly three stable destinations, visible navigation is faster to scan and switch, remains comfortable at phone width, and makes Knightshift feel like a contained private workspace. A drawer would add a tap to every route change and imply hidden complexity that the app does not have. Reconsider a hamburger drawer only if the live top-level workspace count grows beyond four.
