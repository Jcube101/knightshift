...
import { flushSyncOutbox, type SyncClient } from './client'

type TestClient = SyncClient & { collection: ReturnType<typeof vi.fn> }

function client({ userId = 'user-1', existing = null }: { userId?: string | null; existing?: Record<string, unknown> | null } = {}): TestClient {
  ...
  return {
    ...
    collection: vi.fn(() => ({ create, update, getFirstListItem })),
  }
}
